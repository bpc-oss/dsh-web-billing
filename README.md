...
- **展示（浏览器端）**：每条 assistant 消息动作条上的费用角标；会话头部角标显示
  **本会话 + 本日 + 累计**的花费/节省（悬停浮层含每模型金额与 输入/缓存/输出 token
  拆分），与全局汇总一样也可在 `设置 → 费用` 查看完整的今日 / 本月 / 累计 / 账户
  余额 / 按模型 / 会话 / 按天历史。中文界面显示 ¥，英文界面显示 $，也可用
