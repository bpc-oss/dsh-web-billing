...
/** 当前「政策 + 峰谷阶段」的稳定标识，用于探测下一次价格切换（policy since + peak/offPeak/flat）。 */
...
/**
 * 下一次政策或峰谷阶段切换时刻（epoch ms；72 小时内找不到返回 null）。
 * 先按小时探测再二分到秒：既能跨越未来政策生效点，也不假设时区固定。
 * @param timeMs - 起始时刻。
 * @param policies - 政策时间表（默认内置）。
 * @param timezone - 峰谷判定时区。
 * @param windows - 高峰窗口。
 */
export function nextPricingTransition(
  ...
  for (let hour = 1; hour <= 72; hour++) {
    ...
    // 内置政策与峰谷窗口均在整秒边界切换；归一化掉二分留下的亚秒误差。
 ...
 */
export function mergeCounts(target, sample) {
 ...
 * sessions）的 cost 与 calls，报告是否一致。
 * 注意：byProvider 等可能是 Map 或纯对象，必须兼容两者。
 * byDayProvider（v3 的 day×provider×model 维度）在迁移/历史裁剪下必然存在缺口，
 * 该缺口由 /billing/state 的 bdpGap 单独呈现（UI 显式标注），不参与整体 ok。
 */
export function ledgerIntegrity(ledger) {
  const sum = (coll) => {
    let calls = 0, cost = 0;
    const values = coll instanceof Map ? coll.values() : Object.values(coll || {});
    for (const v of values) { calls += v.calls || 0; cost += v.cost || 0; }
    return { calls, cost };
  };
  // byDayProvider 只有活跃天（冻结天被切出）——校验口径需合并 frozenDays（只读快照）；
  // byProvider/byProviderModel/byDay 是历史累计（含冻结），不再加（否则双计）。
  const bdpf = sum(ledger.byDayProvider);
  for (const fd of (ledger.frozenDays ?? new Map()).values()) {
    const fvals = fd.byDayProvider instanceof Map ? fd.byDayProvider.values() : Object.values(fd.byDayProvider ?? {});
    for (const v of fvals) { bdpf.calls += v.calls || 0; bdpf.cost += v.cost || 0; }
  }
  ...
  // totals 是活跃区（冻结天被 freezeDay 减出）——完整性校验用全量（活跃 + frozenTotals）；
  // byProvider/byProviderModel 是历史累计（含冻结），两者口径一致。
  const tFull = { ...t };
  for (const fd of (ledger.frozenDays ?? new Map()).values()) {
    const ft = fd.totals || {};
    for (const field of ["calls", "cost"]) tFull[field] = (tFull[field] ?? 0) + (ft[field] ?? 0);
  }
  // byDay 活跃（冻结切出）——校验口径加 frozen（byDayProvider 快照按日）。
  const bdFull = { ...bd };
  for (const fd of (ledger.frozenDays ?? new Map()).values()) {
    for (const field of ["calls", "cost"]) bdFull[field] = (bdFull[field] ?? 0) + (fd.totals?.[field] ?? 0);
  }
  // 会话聚合求和：用会话级聚合字段（record 时全量维护、不受明细裁剪影响），
  // 而非消息明细求和——明细被 maxMessagesPerSession 裁剪的会话会让该检查永久误报。
  let sess = { calls: 0, cost: 0 };
  for (const [, s] of ledger.bySession ?? []) {
    sess.calls += s.calls || 0;
    sess.cost += s.cost || 0;
  }
  // 容差：小额固定 0.01 + 大额相对 1e-6（避免大账本浮点累计误差误报）。
  const close = (a, b) => {
    const diff = Math.abs(a - b);
    return diff < 0.01 || diff <= Math.abs(b) * 1e-6;
  };
  const checks = [
    { key: "totals_byProvider", ok: close(tFull.cost ?? 0, bp.cost), a: tFull.cost ?? 0, b: bp.cost },
    { key: "byProvider_byProviderModel", ok: close(bp.cost, bpm.cost), a: bp.cost, b: bpm.cost },
    { key: "byDay_byProvider", ok: close(bdFull.cost, bp.cost), a: bdFull.cost, b: bp.cost },
    { key: "sessions_byProvider", ok: close(sess.cost, bp.cost), a: sess.cost, b: bp.cost }
  ];
  const bdpCheck = {
    key: "byDayProvider_byProvider",
    ok: close(bdp.cost, bp.cost),
    a: bdp.cost,
    b: bp.cost,
    gap: bdp.cost - bp.cost
  };
  return { ok: checks.every((c) => c.ok), checks, bdpGapCheck: bdpCheck };
}
...
export const PROMO_MODELS = Object.freeze({
  ...
  // B.AI（api.b.ai）：deepseek-v4-flash 活动免费（用户提供的信息）——放最后，手写覆盖 catalog。
  bai: Object.freeze({
...
export function subscriptionUnitFor(provider, model, nominal, metering = {}, promos = PROMO_MODELS) {
  ...
  // usage-free：仅命中免费清单的模型按 0 计。用户 freeModels 只在**非空**时覆盖
  // 内置活动表——空对象（如用户误清空）应回退内置表，避免免费模型全部失效。
  if (rule.mode === "usage-free") {
    const userModels = rule.freeModels;
    const freeModels = userModels !== null && userModels !== void 0 && Object.keys(userModels).length > 0 ? userModels : promos[provider];
    const freeHit = freeModels !== null && freeModels !== void 0 && freeModels[model] === true;
    if (freeHit) {
...
/**
 * POST 同源校验：Origin（若有）必须与 Host 一致（http/https）。
 * 无 Origin 的本地工具（curl 等）放行；跨站/伪造 Origin 拦截。
 */
