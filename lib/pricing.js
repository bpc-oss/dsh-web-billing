 ...
 * - mode "subscription"：月订阅 —— 调用一律按 0 计（订阅费已付，调用折算为「订阅抵扣」），
 *   `unit` 恒 0、`nominal` 保留官方名义价（用于展示抵扣了多少）。
 * - mode "free"：活动免费 —— 调用一律按 0 计（真正白赚，如 b.ai 免费额度），同样保留名义价。
 * - mode "usage"（或未配置）：按名义价实算花费。
 * 与 `localProviders` 互不冲突（本地部署同样 0 计费，但归类为 local）。
 * @param provider - provider 名。
 * @param nominal - 官方/名义单价。
 * @param metering - provider → { mode: 'usage'|'subscription'|'free', monthly? }。
 * @returns { unit, nominal, metered, mode, monthly } — metered=true 表示该 provider 被计费形式接管。
 */
export function subscriptionUnitFor(provider, nominal, metering = {}) {
  const rule = metering[provider];
  if (rule !== void 0 && rule.mode !== "usage") {
    return {
      unit: { cny: { input: 0, cacheRead: 0, output: 0 }, usd: { input: 0, cacheRead: 0, output: 0 } },
      nominal,
      metered: true,
      mode: rule.mode,
  ...
  return { unit: nominal, nominal, metered: false, mode: "usage" };
}
