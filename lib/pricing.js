 ...
 * 在某一时刻应使用的单价。独立于账本与 HTTP，便于单元测试。
 *
 * 语义约定（与 DeepSeek 官方与 provider 适配器一致）：
 * - input      缓存未命中输入（¥ / 1M tokens）
 * - cacheRead  缓存命中输入（¥ / 1M tokens）
 * - output     输出（¥ / 1M tokens）
 */
...
export function priceFor(model, table) {
  return table[model] ?? table["*"] ?? { input: 0, cacheRead: 0, output: 0 };
}
...
export function resolvePrice(model, baseTable, overrideTable) {
  ...
  if (override[model] !== void 0) return { ...priceFor(model, baseTable), ...override[model] };
  if (baseTable[model] !== void 0) return priceFor(model, baseTable);
  return { ...priceFor(model, baseTable), ...(override["*"] ?? {}) };
}
 ...
 * @param opts - { official, prices(用户表), timezone, peakWindows, policies(官方表) }。
 * @returns { input, cacheRead, output, mode, policy } — mode: 'flat' | 'peak' | 'offPeak'。
 */
export function priceAt(model, timeMs, opts) {
  ...
  if (!official || policies.length === 0) {
    return { ...priceFor(model, prices), mode: "flat", policy: void 0 };
  }
  ...
  const unit = named
    ...
    : { ...priceFor(model, baseTable), ...(prices["*"] ?? {}) };
  return {
    ...unit,
...
/** 按 TokenUsage 与单价计算费用（¥）与 token 拆分。 */
export function costOf(usage, price) {
  const inputTokens = usage.inputTokens ?? 0;
  const cacheReadTokens = usage.cacheReadTokens ?? 0;
  const outputTokens = usage.outputTokens ?? 0;
  const cost = (inputTokens * price.input + cacheReadTokens * price.cacheRead + outputTokens * price.output) / 1e6;
  return { inputTokens, cacheReadTokens, outputTokens, cost };
}
...
export function zeroCounts() {
  return { calls: 0, cost: 0, inputTokens: 0, cacheReadTokens: 0, outputTokens: 0 };
}
