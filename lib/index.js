...
class BillingLedger {
  ...
  load() {
    try {
      ...
      // v2 账本无 byDayProvider：从会话消息明细重建（消息含 time/provider/model）。
      ...
      if (this.byDayProvider.size === 0) {
        for (const [, session] of this.bySession) {
          for (const message of session.messages.values()) {
            ...
            const key = `${day}\x00${provider}\x00${message.model || "unknown"}`;
            let counts = this.byDayProvider.get(key);
            if (counts === void 0) { counts = zeroCounts(); this.byDayProvider.set(key, counts); }
 ...
 */
/**
 * 合并两个**聚合值**（每条已含 calls=N）：所有字段直接相加。
 * 注意：不能用 addCounts —— 其 calls+=1 是「单条记录」语义，合并聚合值会少计 calls。
 */
function mergeCounts(target, sample) {
  target.calls += sample.calls;
