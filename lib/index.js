    ...
    handler: async (req, res) => {
      ...
      if (req.method === "POST" && tail === "/budget") {
        ...
        try {
          ...
          await saveBudgetFile(budgetFile, { enabled: budget.enabled, amount: budget.amount });
      ...
      if (tail === "/state" || tail === "") {
        ...
        // 上月聚合（环比用）。
        const lastMonthKey = monthKey(now - 32 * 864e5);
        ...
        // 高峰占比：从最近流水统计 mode=peak 的调用占比（费用口径）。
        let peakCost = 0;
        let flatCost = 0;
        ...
        const peakShare = peakCost + flatCost > 0 ? peakCost / (peakCost + flatCost) : 0;
