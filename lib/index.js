 ...
 *  free=活动免费（调用按 0 计，真正白赚，如 b.ai 免费额度）。 */
const METERING_SCHEMA = z.object({
  mode: z.union([z.const("usage"), z.const("subscription"), z.const("free")]).default("usage"),
...
function loadMeteringFile(path) {
  try {
    ...
    for (const [provider, rule] of Object.entries(raw ?? {})) {
      const mode = rule?.mode === "subscription" || rule?.mode === "free" ? rule.mode : "usage";
      out[provider] = {
        mode,
