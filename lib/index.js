  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (tail === "/state" || tail === "") {
        ...
        for (const [key, counts] of ledger.byDay) if (key.startsWith(`${lastMonthKey}-`)) addCounts(lastMonthCounts, counts);
        ...
        sendJson(res, 200, {
          ...
          budget: { ...budget },
          history: rangeDays,
          // provider/model 维度用**持久化全量**（ledger.byProviderModel 累计全部历史），
          // 不依赖 recent 流水窗口——窗口外的记录（如较早的 glm-5.2 调用）不会丢失。
          byModel: Object.fromEntries([...[...ledger.byProviderModel.entries()].reduce((acc, [key, counts]) => {
            const sep = key.indexOf("\x00");
            const model = sep >= 0 ? key.slice(sep + 1) : (counts.model || "unknown");
            const cur = acc.get(model);
            if (cur === void 0) { acc.set(model, { ...counts }); }
            else {
              // 合并的是聚合值（每条已含 calls=N）：所有字段直接相加（不能用 addCounts，
              // 其 calls+=1 是单条记账语义）。
              cur.calls += counts.calls;
              cur.cost += counts.cost;
              cur.costUsd += counts.costUsd;
              cur.costNominal += counts.costNominal;
              cur.costNominalUsd += counts.costNominalUsd;
              cur.savings += counts.savings;
              cur.savingsUsd += counts.savingsUsd;
              cur.inputTokens += counts.inputTokens;
              cur.cacheReadTokens += counts.cacheReadTokens;
              cur.outputTokens += counts.outputTokens;
            }
            return acc;
          }, /* @__PURE__ */ new Map()).entries()].map(([model, counts]) => [model, { ...counts, model }]).sort((a, b) => b[1].cost - a[1].cost)),
          byProvider: Object.fromEntries([...ledger.byProvider.entries()].map(([provider, counts]) => [provider, {
            ...counts,
            derived: !configuredProviders.has(provider) && !(provider in CODING_PLAN_PRICING)
          }]).sort((a, b) => b[1].cost - a[1].cost)),
          configuredProviders: [...configuredProviders],
          byProviderModel: Object.fromEntries([...ledger.byProviderModel.entries()]
            .map(([key, counts]) => {
              const sep = key.indexOf("\x00");
