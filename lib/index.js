...
function loadMeteringFile(path) {
  try {
    ...
    for (const [provider, rule] of Object.entries(raw ?? {})) {
      ...
      out[provider] = {
        mode,
        monthly: Number.isFinite(Number(rule?.monthly)) ? Number(rule.monthly) : 0
      };
    }
    return out;
  } catch (error) {
    ...
    handler: async (req, res) => {
      ...
      if (req.method === "POST" && tail === "/metering") {
        ...
        try {
          ...
          for (const [provider, rule] of Object.entries(patch ?? {})) {
            const mode = rule?.mode === "subscription" || rule?.mode === "free" || rule?.mode === "local" ? rule.mode : "usage";
            const monthly = Number.isFinite(Number(rule?.monthly)) ? Number(rule.monthly) : 0;
      ...
      if (tail === "/state" || tail === "") {
        ...
        sendJson(res, 200, {
          ...
          metering: Object.fromEntries(Object.entries(effectiveMetering).map(([provider, rule]) => [provider, { ...rule, monthlyUsd: rule.mode === "subscription" ? rule.monthly : 0 }])),
