window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var BillingController = class {
			...
			async refresh(range) {
				try {
					...
					if (typeof range === "string" && range.length > 0) this.currentRange = range;
		...
		* @returns SVG 折线图。
		*/
		function TrendChart({ trend, symbol, t }) {
			const W = 560;
			const H = 110;
			const PAD = 6;
			...
			const line = (key, color) => {
				...
				return react_jsx_runtime.jsx("polyline", {
					fill: "none",
					// CSS 变量必须走 style（SVG presentation attribute 不解析 var()）。
					style: { stroke: color },
					strokeWidth: 1.6,
			...
			// 首/中/末标签；n<3 时中标签与末标签重叠，只渲染首/末。
			const labelIndexes = n < 3 ? [0, n - 1] : [0, Math.floor(n / 2), n - 1];
			const labels = labelIndexes.map((i, idx) => {
				const d = trend[i];
				const anchor = idx === 0 ? "start" : idx === labelIndexes.length - 1 ? "end" : "middle";
		...
		*/
		function SettingsBillingSection({ useBilling, useLocale, t, refresh, useSessions }) {
			...
			// 回本率：把 ¥月费换算到当前显示币种后再与节省同口径相除，避免 USD/CNY 混算。
			const subMonthlyDisplay = currency === "USD" ? subMonthly / (coding?.rate ?? 7.2) : subMonthly;
			const recoveryRate = subMonthlyDisplay > 0 ? subSaved / subMonthlyDisplay : null;
				...
				children: [
					...
					react_jsx_runtime.jsx("div", {
						...
						children: [
							...
							react_jsx_runtime.jsx("div", { className: css_default.setCard, children: [
								...
								react_jsx_runtime.jsx("div", { className: css_default.setCardValue, children: recoveryRate === null ? "—" : `×${recoveryRate.toFixed(1)}` }),
								react_jsx_runtime.jsx("div", { className: css_default.setCardSub, children: `${t("source.monthly")} ${formatCost(symbol, subMonthlyDisplay)}` })
							] }),
							react_jsx_runtime.jsx("div", { className: css_default.setCard, children: [
							...
							react_jsx_runtime.jsx("div", { className: css_default.setCard, children: [
								react_jsx_runtime.jsx("p", { className: css_default.setCardLabel, children: t("insight.rangeTokens") }),
								react_jsx_runtime.jsx("div", { className: css_default.setCardValue, children: fmtTok((totals.inputTokens ?? 0) + (totals.cacheReadTokens ?? 0) + (totals.outputTokens ?? 0)) }),
								react_jsx_runtime.jsx("div", { className: css_default.setCardSub, children: `${t("input")} ${fmtTok(totals.inputTokens ?? 0)} · ${t("cache")} ${fmtTok(totals.cacheReadTokens ?? 0)} · ${t("output")} ${fmtTok(totals.outputTokens ?? 0)}` })
							] })
						]
					}),
					// ── 每日趋势图（SVG 折线：花费/节省） ──
					trend.length > 0 && react_jsx_runtime.jsx("div", {
						...
						children: [
							...
							react_jsx_runtime.jsx(TrendChart, { trend, symbol, t })
						]
					}),
					// ── 月度预算 ──
					react_jsx_runtime.jsx("div", {
										...
										onClick: async () => {
											...
											try {
												...
												if (result?.ok === true) {
													...
													if (typeof refresh === "function") refresh(range);
												}
											} catch { /* 静默 */ }
										},
					...
					// ── 白嫖推荐：有免费模型的 provider，一键设为「按量+可白嫖」 ──
					Object.keys(promos).length > 0 && react_jsx_runtime.jsx("p", { className: css_default.setSectionTitle, children: t("promo.title") }),
					Object.keys(promos).length > 0 && react_jsx_runtime.jsx("div", {
						children: [
							Object.entries(promos).map(([provider, freeModels]) => {
								const modelNames = Object.keys(freeModels).filter((m) => freeModels[m] === true);
								if (modelNames.length === 0) return null;
								const rule = meteringDraft === null ? (metering[provider] ?? { mode: "usage" }) : (meteringDraft[provider] ?? { mode: "usage" });
								const enabled = rule.mode === "usage-free";
								const alreadySubOrFree = rule.mode === "subscription" || rule.mode === "free";
								return react_jsx_runtime.jsxs("div", {
									className: css_default.srcGroup,
									children: [
										react_jsx_runtime.jsxs("div", {
											...
											children: [
												react_jsx_runtime.jsxs("span", {
													className: css_default.srcTitle,
													children: [
														react_jsx_runtime.jsx("span", { className: css_default.srcDot, style: { background: "#0ea5e9" } }),
														provider,
														react_jsx_runtime.jsx("span", { className: css_default.srcCalls, children: `${t("promo.freeCount")} ${modelNames.length}` })
													]
												}),
												enabled
													? react_jsx_runtime.jsx("span", { className: `${css_default.setCardSub} ${css_default.save}`, children: t("promo.enabled") })
													: alreadySubOrFree
														? react_jsx_runtime.jsx("span", { className: css_default.srcCalls, children: t("promo.inUse") })
														: react_jsx_runtime.jsx("button", {
														type: "button",
														className: css_default.foldBtn,
														onClick: () => {
															setMeteringDraft((prev) => {
																...
																base[provider] = { mode: "usage-free", monthly: 0, freeModels: modelNames.reduce((acc, m) => { acc[m] = true; return acc; }, {}) };
																return base;
															});
															setMeteringSaved(false);
														},
														children: t("promo.enable")
													})
											]
										}),
										react_jsx_runtime.jsx("div", {
											className: css_default.srcNote,
											children: modelNames.slice(0, 6).join(" · ") + (modelNames.length > 6 ? ` · …${modelNames.length - 6}` : "")
										})
									]
								}, provider);
							})
		...
		const zh = {
			...
			"insight.recovery": "累计回本倍数",
			"insight.peakShare": "高峰占比",
			"insight.peakHint": "按最近流水估算",
			"insight.rangeTokens": "范围 Token",
			"trend.title": "每日趋势",
			"trend.max": "峰值",
			"budget.title": "月度预算",
			"budget.enable": "启用",
			...
			"promo.freeCount": "免费模型",
			"promo.enable": "设为按量+可白嫖",
			"promo.enabled": "已启用",
			"promo.inUse": "已按订阅/免费处理",
			"pricing.auto": "官方政策自动计价",
			"pricing.custom": "自定义价格",
			"pricing.peakNow": "当前为高峰期",
			"pricing.offPeakNow": "当前为峰谷期",
			"balance": "账户余额",
			"balance.title": "显示账户余额",
			"balance.on": "已开启",
			"balance.off": "已关闭",
			"balance.hint": "即时生效，无需重启",
		...
		const en = {
			...
			"insight.recovery": "Cumulative recovery",
			"insight.peakShare": "Peak share",
			"insight.peakHint": "est. from recent ledger",
			"insight.rangeTokens": "Range tokens",
			...
			"promo.freeCount": "free models",
			"promo.enable": "Enable usage+free",
			"promo.enabled": "Enabled",
			"promo.inUse": "Already subscription/free",
			...
			"balance.title": "Show account balance",
			"balance.on": "On",
			"balance.off": "Off",
			"balance.hint": "takes effect immediately, no restart",
