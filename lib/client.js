window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* （官方 / 本地 / coding plan），浮层额外显示当前官方计价时段（高峰/空闲）。
		...
		*/
		function SessionCostBadge({ sessionId, useBilling, useLocale, refreshSession, t }) {
			...
			for (const message of messages) {
				...
				// 本对话分模型：累计金额 + token（缓存命中率单算）。
				let bucket = byModel.get(model);
				if (bucket === void 0) {
					bucket = { cost: 0, costUsd: 0, savings: 0, savingsUsd: 0, input: 0, cacheRead: 0, output: 0, calls: 0 };
			...
			const modelRows = [...byModel.entries()]
				.sort((a, b) => (b[1].cost - b[1].savings) - (a[1].cost - a[1].savings))
				.slice(0, 8)
				.map(([model, counts]) => {
							...
							children: [
								...
								react_jsx_runtime.jsx("div", {
									...
									children: [
										...
										gridCell(t("today"), bucketToday, true),
										gridCell(t("total"), bucketTotal, false),
										gridCell(t("total"), bucketTotal, true)
									]
								}),
