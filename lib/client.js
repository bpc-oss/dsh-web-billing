	...
	factory: (require) => {
		...
		*/
		function SessionCostBadge({ sessionId, useBilling, useLocale, refreshSession, t }) {
			...
			// 浮层定位：跟随主内容区（会话列）右上角，而不是视口最右（sidebar 区域）。
			...
			// 打开时测量主内容区（会话列容器）右缘，浮层定位到其中（不受 sidebar 影响）。
			// open 期间每 500ms 重测——sidebar 开/关变化时浮层位置自动跟随，无需重新悬停。
			react.useLayoutEffect(() => {
				if (!open) return;
				const measure = () => {
					// 检测 sidebar 是否打开：折叠态是 ~56px 窄条，打开态 ≥150px。
					let sidebarVisible = false;
					try {
						const sidebars = document.querySelectorAll("[class*='sidebar'], [class*='Sidebar'], [data-sidebar]");
						for (const s of sidebars) {
							const r = s.getBoundingClientRect();
							if (r.width > 150 && r.width < window.innerWidth * 0.6) { sidebarVisible = true; break; }
						}
					} catch { /* 忽略 */ }
					let next = 16;
					if (sidebarVisible) {
						// 有 sidebar：锚定主内容区（会话列）右缘，避开 sidebar 区域。
						const cols = document.querySelectorAll("[class*='DdGtAG_column'], [class*='chatContent'], [data-dsh-chat]");
						let el = null;
						for (const c of cols) { if (c.getBoundingClientRect().width > 200) { el = c; break; } }
						if (el !== null) {
							const rc = el.getBoundingClientRect();
							next = Math.max(16, window.innerWidth - rc.right);
						} else {
							next = Math.max(16, (window.innerWidth - 748) / 2);
						}
					}
					setPanelPos((prev) => (prev !== null && prev.right === next ? prev : { right: next }));
				};
				measure();
				...
				children: react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
					children: [
						...
						open && react_dom.createPortal(react_jsx_runtime.jsx("div", {
							...
							style: panelPos === null ? void 0 : { right: panelPos.right },
							onMouseEnter: () => {
								if (closeTimer.current !== null) {
									clearTimeout(closeTimer.current);
									closeTimer.current = null;
									...
									react_jsx_runtime.jsx("div", {
										...
										children: SOURCE_META.map((meta) => {
											...
											return react_jsx_runtime.jsxs("span", {
												...
												style: isSavingGroup ? { color: meta.color } : void 0,
												children: [
