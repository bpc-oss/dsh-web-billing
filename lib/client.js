	...
	factory: (require) => {
		...
		*/
		function SessionCostBadge({ sessionId, useBilling, useLocale, refreshSession, t }) {
			...
			// 节省明细（与花费平行的统计）：本会话 / 今日 / 本月 / 累计，任一 >0 才显示。
			const todaySavings = today === void 0 ? 0 : savedOf(today);
			const monthSavings = month === void 0 ? 0 : savedOf(month);
			const savingsRows = [];
			if (sessionSavings > 0 || todaySavings > 0 || monthSavings > 0 || totalSavings > 0) {
				savingsRows.push([t("saved"), formatCost(symbol, sessionSavings)]);
				if (today !== void 0) savingsRows.push([`${t("savedToday")} (${String(today.date).slice(5)})`, formatCost(symbol, todaySavings)]);
				if (month !== void 0) savingsRows.push([`${t("savedMonth")} (${month.key})`, formatCost(symbol, monthSavings)]);
				savingsRows.push([t("savedTotal"), formatCost(symbol, totalSavings)]);
			}
							...
							children: [
								...
								open && react_jsx_runtime.jsxs("div", {
									...
									children: [
										...
										savingsRows.length > 0 && react_jsx_runtime.jsx("div", {
											className: css_default.title,
											children: t("savingsSection")
										}),
										savingsRows.map(([key, value]) => react_jsx_runtime.jsxs("div", {
											className: css_default.row,
											children: [
