	...
	factory: (require) => {
		...
		*/
		function SessionCostBadge({ sessionId, useBilling, useLocale, refreshSession, t }) {
			...
			// 浮层移到 body 顶层：脱离会话头部（position:relative;z-index:2 创建层叠上下文）
			// 的限制，避免被 sidebar 盖住。useLayoutEffect 在绘制前同步移动，无闪烁。
			react.useLayoutEffect(() => {
				if (open && panelRef.current !== null && panelRef.current.parentElement !== document.body) {
					document.body.appendChild(panelRef.current);
				}
			}, [open]);
			// 追踪鼠标坐标（关闭判断用 elementFromPoint）。
			const trackMouse = (e) => {
			...
			const closeSoon = () => {
				if (open && !clickLocked.current) {
					...
					closeTimer.current = setTimeout(() => {
						closeTimer.current = null;
						// 浮层已移到 body：鼠标若仍在浮层上（elementFromPoint 命中）则不关闭，
						// 否则关闭——避免鼠标移到浮层时被误关。
						try {
							const over = document.elementFromPoint(mouseX.current, mouseY.current);
							const panel = panelRef.current;
							if (panel !== null && over !== null && (panel === over || panel.contains(over))) {
								closeSoon();
								return;
							}
						} catch { /* 忽略 */ }
