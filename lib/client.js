window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const tagId = "@dsh-local/dsh-web-billing/billing.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@dsh-local/dsh-web-billing";
		...
		*/
		function MessageCostChip({ messageId, sessionId, useBilling, refreshSession, t }) {
			...
			const symbol = view.value?.symbol ?? "¥";
			return react_jsx_runtime.jsx("span", {
				className: css_default.chip,
				title: detail,
				children: formatCost(symbol, message.cost)
			});
		}
		//#endregion
		...
		*/
		function SessionCostBadge({ sessionId, useBilling, refreshSession, t }) {
			const view = useBilling((snapshot) => snapshot);
			...
			if (session === void 0 || session.cost <= 0) return null;
			const symbol = view.value?.symbol ?? "¥";
			const totals = view.value?.totals;
			const today = view.value?.today;
			const month = view.value?.month;
			const rows = [];
			if (totals !== void 0) rows.push([t("total"), formatCost(symbol, totals.cost)]);
			if (today !== void 0) rows.push([`${t("today")} (${String(today.date).slice(5)})`, formatCost(symbol, today.cost)]);
			if (month !== void 0) rows.push([`${t("month")} (${month.key})`, formatCost(symbol, month.cost)]);
			const byModel = view.value?.byModel;
			const modelRows = byModel === void 0 ? [] : Object.entries(byModel)
				.sort((a, b) => b[1].cost - a[1].cost)
				.slice(0, 8)
				.map(([model, counts]) => [model, formatCost(symbol, counts.cost)]);
			...
			const balanceRow = balance === void 0 || balance.status === "disabled" || balance.status === "idle" ? null
				: balance.status === "ready" && balance.balance !== void 0
					? [t("balance"), formatCost(symbol, balance.balance.total), `${t("balance.granted")} ${formatCost(symbol, balance.balance.granted)} · ${t("balance.toppedUp")} ${formatCost(symbol, balance.balance.toppedUp)}`]
						...
						children: react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
							children: [
								react_jsx_runtime.jsx("button", {
									...
									"aria-expanded": open,
									title: `${t("sessionCost")} · ${t("calls")} ${formatNumber(session.calls)}`,
									onClick: () => {
										setOpen(!open);
									},
									children: formatCost(symbol, session.cost)
								}),
								...
								open && react_jsx_runtime.jsxs("div", {
									...
									children: [
										react_jsx_runtime.jsx("div", {
											...
											children: [
												react_jsx_runtime.jsx("span", { className: css_default.key, children: t("sessionCost") }),
												react_jsx_runtime.jsx("span", { className: css_default.value, children: formatCost(symbol, session.cost) })
											]
										}),
		...
		/**
		* 计费插件浏览器端：在 assistant 消息动作条与会话头部工具条渲染人民币费用，
		* 数据来自 host 的 /billing 只读端点（轮询 5s + 会话级按需拉取）。
		* @module @dsh-local/dsh-web-billing/client
		*/
		/** 字典命名空间。 */
		const NS = "billing";
		/** 需要的服务：插槽注册与本地化。 */
