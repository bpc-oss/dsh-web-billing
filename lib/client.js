	...
	factory: (require) => {
		...
		*/
		var BillingController = class {
			...
			constructor() {
				...
				this.startPolling();
				// 页面隐藏时暂停轮询（避免后台持续请求 /billing）；可见时恢复并立即刷新。
			...
			onVisibilityChange = () => {
				...
				if (document.hidden) {
					...
					this.startPolling();
					void this.refresh(this.currentRange);
				}
			};
			...
			refreshSession(sessionId) {
				...
				// 已知会话集只增不减 → 超出 100 后按 FIFO 淘汰，防止长期会话泄漏。
				if (this.knownSessions.size > 100) {
					const oldest = this.knownSessions.values().next().value;
					if (oldest !== void 0) this.knownSessions.delete(oldest);
				}
		...
		*/
		function SessionCostBadge({ sessionId, useBilling, useLocale, refreshSession, t }) {
			...
			// 分模型优先用 host 的 session.models（record 时累计、永不裁剪，与 session 总账一致）；
			// 无该字段（旧 host）时回退从 messages 聚合。
			const hostModels = session.models;
			if (hostModels !== void 0 && typeof hostModels === "object" && Object.keys(hostModels).length > 0) {
				for (const [, m] of Object.entries(hostModels)) {
					const model = m.model ?? "unknown";
					const bucket = {
						provider: m.provider ?? "?",
						model,
						source: m.source ?? "official",
						cost: m.cost ?? 0, costUsd: m.costUsd ?? 0,
						savings: m.savings ?? 0, savingsUsd: m.savingsUsd ?? 0,
						input: m.inputTokens ?? 0, cacheRead: m.cacheReadTokens ?? 0,
						output: m.outputTokens ?? 0, calls: m.calls ?? 0
					};
					byModel.set(`${m.provider ?? "?"}\u0009${model}`, bucket);
			...
			for (const message of messages) {
				...
				// 本对话分模型（仅当无 host models 时从这里聚合；有则上面已用完整值）。
				if (hostModels === void 0 || Object.keys(hostModels).length === 0) {
					...
					children: [
						...
						open && react_dom.createPortal(react_jsx_runtime.jsx("div", {
							...
							children: [
								...
								// 分模型缺口：正=分模型和小于会话总账（历史裁剪/迁移），
								// 负=分模型膨胀（双计异常）——两者都显式提示，让对账闭合。
								(() => {
									const gap = session.modelGap;
									const gapCost = currency === "USD" ? gap?.costUsd : gap?.cost;
									if (gap === void 0 || gapCost === 0) return null;
									return react_jsx_runtime.jsxs("div", {
										className: css_default.phaseRow,
										children: [
											react_jsx_runtime.jsx("span", {
												className: css_default.phaseDot,
												style: { background: "var(--dsw-alias-state-warn-primary, #f59e0b)" }
											}),
											react_jsx_runtime.jsx("span", {
												className: css_default.phaseText,
												children: `${t("modelGap")} ${formatCost(symbol, Math.abs(gapCost))}${gapCost < 0 ? ` ${t("modelGapInflated")}` : ""}`
											})
										]
									});
								})(),
								...
								(() => {
									...
									// ready 显示；error 但保留 proven 缓存（瞬时失败不抹掉余额）时也显示。
									if (balEnabled === false || bal === void 0 || (bal.status !== "ready" && bal.status !== "error") || bal.balance === void 0) return null;
		...
		*/
		function SettingsBillingSection({ useBilling, useLocale, t, refresh, useSessions }) {
			...
			// 真累计（全时口径，host 独立于所选范围下发）——「累计」卡展示此值；旧 host
			// 无该字段时回退范围 totals（至少不是 ¥0）。
			const lifetime = value?.lifetime ?? totals;
			...
			const configuredProviders = Array.isArray(value?.configuredProviders) ? value.configuredProviders : [];
			...
			const balanceNode = balance === void 0 || balance.status === "disabled" ? null
				// ready 显示；error 但保留有 proven 缓存（瞬时失败不抹掉）时也显示，副标题标注缓存时间。
				: (balance.status === "ready" || balance.status === "error") && balance.balance !== void 0
					? (() => {
						...
						const stale = balance.status === "error";
						return info === null ? react_jsx_runtime.jsx("p", { className: css_default.setHint, children: t("balance.unavailable") })
							: react_jsx_runtime.jsx("div", {
								...
								children: [
									...
									react_jsx_runtime.jsx("div", { className: css_default.setCardSub, children: stale ? t("balance.stale") : `${t("balance.granted")} ${formatCost(infoSymbol, info.granted)} · ${t("balance.toppedUp")} ${formatCost(infoSymbol, info.toppedUp)}` })
								]
							});
					})()
				...
				children: [
					...
					react_jsx_runtime.jsx("div", {
						children: [
							...
							// 添加 provider 收费形式：候选 = 账本出现过的 ∪ DSH 已配置的 provider，
							// 排除已配置 metering 的（新加的 provider 未产生计费记录也能配置）。
							(() => {
								const seen = new Set([...Object.keys(byProvider), ...configuredProviders]);
								const candidates = [...seen].filter((p) => metering[p] === void 0).sort();
						...
						children: [
							react_jsx_runtime.jsxs("div", {
								...
								children: [
									...
									// 重估缺口：被裁剪的历史消息无法按新价重算，重估后账单缩水。
									value?.repriceGap !== null && value?.repriceGap !== void 0 && value.repriceGap.calls > 0 && react_jsx_runtime.jsx("div", {
										className: css_default.setCardSub,
										style: { color: "var(--dsw-alias-state-warn-primary, #f59e0b)" },
										children: `${t("integrity.repriceGap")} ${formatNumber(value.repriceGap.calls)} 条 / ${formatCost(symbol, value.repriceGap.cost)}`
									})
								]
							})
						]
		...
		const zh = {
			...
			"integrity.repriceGap": "重估缺口（被裁剪历史无法按新价重算）",
			"modelGap": "分模型缺（已裁剪历史未计入）",
			"modelGapInflated": "·分模型膨胀（异常）",
			...
			"balance.stale": "余额来自最近一次成功查询（当前查询失败，稍后自动重试）",
			"balance.granted": "赠金",
			"balance.toppedUp": "充值",
			"local": "本地模型",
			"cost": "花费",
		...
		const en = {
			...
			"integrity.repriceGap": "Reprice gap (trimmed history cannot be repriced)",
			"modelGap": "Model split gap (trimmed history not in per-model rows)",
			"modelGapInflated": "·per-model inflated (anomaly)",
			...
			"balance.stale": "Balance from the last successful query (current query failed; retrying automatically)",
