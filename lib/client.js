	...
	factory: (require) => {
		...
		*/
		function apply(ctx) {
			...
			// Blank sessions deliberately omit the session header, so a billing
			// badge mounted there becomes invisible exactly when a user opens the
			// app to check cumulative usage. The composer tool row survives the
			// blank -> active transition and is therefore the stable surface.
			tearDown.push(ctx.slots.inject("conversation.input.right", () => {
				const dispose = ctx.slots.register({
