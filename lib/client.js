		...
		function SessionCostBadge({ sessionId, useBilling, useLocale, refreshSession, t }) {
			...
			react.useLayoutEffect(() => {
				...
				const measure = () => {
					...
					if (!sidebarVisible) {
						// 无 sidebar：贴视口右上角（right:16px）。
						setPanelPos({ right: 16 });
						return;
					}
					...
					if (el === null) {
						setPanelPos({ right: Math.max(16, (window.innerWidth - 748) / 2) });
						return;
					}
					const rc = el.getBoundingClientRect();
					const right = Math.max(16, window.innerWidth - rc.right);
					setPanelPos({ right });
				};
				measure();
