...
export class BalanceFetcher {
  ...
  /**
   * 运行时开关：关闭时停止轮询并清快照为 disabled（可再次开启）；开启时恢复轮询。
   * 区别于 dispose（永久关闭、卸载用）。
   * @param enabled - 是否启用余额查询。
   */
  setEnabled(enabled) {
    if (enabled) {
      if (this.timer === null && !this.disposed) this.start();
      return;
    }
    if (this.timer !== null) clearInterval(this.timer);
    this.timer = null;
    if (!this.disposed) this.publish({ status: "disabled" });
  }
  ...
  async refresh() {
    ...
    this.inFlight = this.fetchOnce()
      ...
      .catch((error) => {
        // 瞬时失败（超时/凭证服务抖动）不得抹掉已被官方验证过的余额：
        // 保留最近一次 proven 值作为 stale 缓存，周期刷新继续重试。
        this.publish({
          status: "error",
