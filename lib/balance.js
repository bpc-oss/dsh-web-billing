...
/** 首选币种（按此顺序挑选 balance_infos 条目）。 */
const CURRENCY_PREFERENCE = ["CNY", "USD"];
 ...
 * @returns 规范化余额视图；响应形状不合法时返回 null。
 */
export function parseBalanceResponse(json) {
  ...
  const preferred = CURRENCY_PREFERENCE.map((currency) => infos.find((info) => info.currency === currency))
    .find((info) => info !== void 0) ?? infos[0];
  return {
    isAvailable: json.is_available === true,
    currency: preferred.currency,
    total: toNumber(preferred.total_balance),
    granted: toNumber(preferred.granted_balance),
    toppedUp: toNumber(preferred.topped_up_balance)
  };
}
